...
const Navbar = () => {
  ...
  // Get full AI availability state including recheck function
  const { 
    isAvailable: isAIAvailable, 
    isChecking: isAIChecking,
    forceRefresh: recheckAI 
  } = useAIAvailability();
    ...
    <>
              ...
              {!isAIAvailable && (
                ...
                >
                  <p className="text-xs mb-2" style={{ color: 'var(--text-secondary)' }}>
                    Local AI is not available. Start Ollama to enable AI features.
                  </p>
                  <button
                    onClick={recheckAI}
                    ...
                    style={{
                      backgroundColor: isAIChecking ? 'var(--bg-secondary)' : 'var(--accent)',
                      color: isAIChecking ? 'var(--text-secondary)' : '#000',
                      opacity: isAIChecking ? 0.6 : 1,
                      cursor: isAIChecking ? 'not-allowed' : 'pointer'
                    }}
                  >
                    {isAIChecking ? (
                      <>
      ...
      >
        ...
        <div className="p-4 space-y-3" style={{ borderTop: '1px solid var(--border-color)' }}>
          ...
          {/* AI Availability Notice - Shows when AI is unavailable */}
          {!isAIAvailable && (
            <div 
            ...
            >
              ...
              <button
                onClick={recheckAI}
                disabled={isAIChecking}
                className="w-full px-3 py-2 rounded-lg transition-all duration-200 flex items-center justify-center gap-2 text-sm"
                style={{ 
                  backgroundColor: isAIChecking ? 'var(--bg-card-hover)' : 'var(--accent-dim)', 
                  color: isAIChecking ? 'var(--text-muted)' : 'var(--accent)',
                  border: `1px solid ${isAIChecking ? 'var(--border-color)' : 'var(--accent)'}`,
                  opacity: isAIChecking ? 0.6 : 1,
                }}
                title="Check if Ollama is now available"
              >
                {isAIChecking ? (
                  <>
          ...
          {/* AI Assistant Button - Only shows when AI is available */}
          {isAIAvailable && (
            <button
